<template>
  <!--    <el-dialog-->
  <!--            :visible="modeler && nodeVisible" :close-on-click-modal="false" title="编辑" width="700px" append-to-body-->
  <!--            destroy-on-close-->
  <!--            @close="TOGGLE_NODE_VISIBLE(false)"-->
  <!--    >-->
...
export default {
  ...
  methods: {
    ...mapMutations(['TOGGLE_NODE_VISIBLE']),
    ...
    handleModeler () {
      ...
      this.modeler.on('commandStack.changed', async () => {
        const res = await _this.modeler.saveXML({ format: true })
        console.log(res)
        _this.$emit('updateXml', res.xml)
      })
      ...
      this.modeler.on('element.click', e => {
        ...
        if (element.type == this.modeler._definitions.rootElements[0].$type) {
          ...
          if (element.type == 'bpmn:UserTask') {
            const _businessObject = element.businessObject
